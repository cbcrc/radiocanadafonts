...
# $ gifsicle -i animated-vf-specimen-002.gif -O3 --colors 32 -o anim-002.gif
...
for frame in range(F - 1):
    ...
    text("Latin Core Character Set: Width Axis (wdth) = ", (M, U * 27.25))
    text(str(int(varWdth)), (M * 9.3, U * 27.25))
    text("Latin Plus Character Set: Width Axis (wdth) = ", (M, U * 19.25))
    text(str(int(varWdth)), (M * 9.3, U * 19.25))
    stroke(1)
    strokeWidth(2)
    line((M, H - M), (W - M, H - M))
...
saveImage("animated-vf-specimen-002.gif")
