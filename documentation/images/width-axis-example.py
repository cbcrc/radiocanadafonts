...
for frame in range(F - 1):
    ...
    text("Radio-Canada", ( M+(-10), M+(U*0) ))
    step += 0.1
    # AUXILIARY TEXT INFO
    font("InputMonoCompressed-Regular")
    stroke(None)
    fontSize(U / 1.5)
    text("Radio Canada Variable Font: Width Axis Range (75 - 100) wdth = ", (M, U * 5.25))
    text(str(int(varWdth)), (M * 12.2, U * 5.25))
    stroke(1)
    strokeWidth(2)
    line((M, H - M), (W - M, H - M))

# SAVE THE ANIMATION IN THIS SCRIPT'S DIRECTORY
saveImage("width-axis-example.gif")
